...
def mobile_net_block(inputs, first_filters, alpha):
    ...
    # x = _inverted_residual_block(x, 40, (3, 3), t=2, alpha=alpha, strides=2, n=2)
    x = _inverted_residual_block(x, 48, (3, 3), t=2, alpha=alpha, strides=2, n=2)

    if alpha > 1.0:
        last_filters = _make_divisible(56 * alpha, 8)
    else:
    	last_filters = 40
...
def model_mobnet(num_classes, input_shape=[128, None, 2], num_filters=24, wd=1e-3, alpha=1):
    ...
    OutputPath = Dropout(0.3, name='Dropout2')(OutputPath)
...
# 1. One extra dropout layer after mobile net block with 20% dropouts.
# 2. Reduced last filters to 56*alpha or 40 instead of 80*alpha or 56.
