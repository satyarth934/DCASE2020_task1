...
# _inverted_residual_block n=2 instead of 3 for a smaller model
def mobile_net_block(inputs, first_filters, alpha):
    x = _conv_block(inputs, first_filters, (3, 3), strides=(2, 2))
    x = _inverted_residual_block(x, 24, (3, 3), t=2, alpha=alpha, strides=2, n=1)
    x = _conv_block(x, 16, (1, 1), strides=(1, 1))
    x = _conv_block(x, 8, (1, 1), strides=(1, 1))
    # x = _inverted_residual_block(x, 40, (3, 3), t=2, alpha=alpha, strides=2, n=1)
    # x = _inverted_residual_block(x, 48, (3, 3), t=2, alpha=alpha, strides=2, n=1)

    if alpha > 1.0:
        last_filters = _make_divisible(48 * alpha, 8)
    else:
    	last_filters = 32
...
def model_mobnet(num_classes, input_shape=[128, None, 2], num_filters=24, wd=1e-3, alpha=1):
    ...
    # OutputPath = Dropout(0.2, name='Dropout1')(MobilePath)
    ...
    OutputPath = Dropout(0.3, name='Dropout2')(MobilePath)
...
# Mobnet smaller 3 description
# 1. Removed first resnet_layer and the dropout before it.
# 2. Reduced the number of last filters to 48 and 32 instead of 56 and 40
